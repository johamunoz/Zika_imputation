...
nindex <- as.data.table(readxl::read_xlsx(here('1_Input_data','Index.xlsx'),sheet="Sheet2")) #Specifies the order of list of variables included only in imputation model 
index <- index[Bkey%in%c(1,2)]  #both Lauren and Anneke list
...
#data[,inf_sex := ifelse(inf_sex==3,NA,inf_sex)] # Assigning 3 to NA
...
Negative values of the following variables were assigned to NA @Johanna I commented some of these out as there are no negative values for some in the new data anymore. If you agree, you can delete these lines!
...
```{r}
data[,birth:=ifelse(!is.na(birth_ga),1,NA)] #birth indicator from birthga
